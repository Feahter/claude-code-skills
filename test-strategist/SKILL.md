...
description: |
  ...
  关键分流：要产出**判断**（风险、优先级、验证义务、处置决定）用本 skill；要产出**测试代码**（选择器、断言、mock、fixture、spec 文件）用 playbook；要给已有测试**打分定级**用 e2e-test-quality。
  不适用：写或改测试代码、排查单个测试为何挂（用 playbook 的 trace-debug / flaky）、纯功能 bug 定位（用 diagnose）。
---
